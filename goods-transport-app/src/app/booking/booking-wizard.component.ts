...
@Component({
  ...
  `,
  styles: [
    ".wizard-shell { background: var(--panel-bg); padding: 1.5rem; border-radius: 1.25rem; display: flex; flex-direction: column; gap: 1rem; border: 1px solid var(--border); box-shadow: var(--shadow); }",
    ".wizard-head { display: flex; justify-content: space-between; gap: 1rem; align-items: start; }",
    ".wizard-head h2 { margin: 0.25rem 0 0.35rem; color: var(--text); }",
    ...
    ".wizard-summary { background: linear-gradient(135deg, color-mix(in srgb, var(--brand-contrast) 60%, white), color-mix(in srgb, var(--brand-contrast) 85%, var(--brand) 15%)); border: 1px solid color-mix(in srgb, var(--brand) 18%, transparent); padding: 0.9rem 1rem; border-radius: 1rem; min-width: 220px; }",
    ".wizard-summary span { display: block; color: var(--muted); }",
    ".wizard-summary strong { font-size: 1.45rem; color: var(--text); }",
    ".steps { display: flex; gap: 0.6rem; flex-wrap: wrap; }",
    ".step { padding: 0.55rem 0.95rem; border-radius: 999px; background: color-mix(in srgb, var(--brand-contrast) 55%, white); font-size: 0.82rem; color: var(--brand-strong); border: 1px solid color-mix(in srgb, var(--brand) 12%, transparent); }",
    ".step.active { background: linear-gradient(135deg, var(--brand), var(--brand-strong)); color: var(--inverse-text); border-color: transparent; }",
    ".step-form { display: flex; flex-direction: column; gap: 0.9rem; }",
    "label { display: flex; flex-direction: column; gap: 0.25rem; font-size: 0.85rem; color: color-mix(in srgb, var(--text) 82%, white); font-weight: 600; }",
    "input, select, textarea { border: 1px solid var(--border); border-radius: 0.75rem; padding: 0.65rem 0.8rem; font-size: 1rem; color: var(--text); background: white; font: inherit; }",
    "input:focus, select:focus, textarea:focus { outline: none; border-color: var(--brand-strong); box-shadow: 0 0 0 4px color-mix(in srgb, var(--brand) 14%, transparent); }",
    ".estimate, .review { background: var(--panel-muted-bg); padding: 0.95rem; border-radius: 1rem; border: 1px solid var(--border); color: var(--muted); }",
    ".estimate p, .review p, .review h3 { color: inherit; }",
    ".review h3 { color: var(--text); }",
    ".actions { display: flex; justify-content: flex-end; gap: 0.75rem; }",
    "button { padding: 0.75rem 1.2rem; border: none; border-radius: 999px; background: linear-gradient(135deg, var(--brand), var(--brand-strong)); color: var(--inverse-text); cursor: pointer; box-shadow: 0 16px 30px color-mix(in srgb, var(--brand) 20%, transparent); }",
    "button:disabled { background: color-mix(in srgb, var(--text) 22%, white); cursor: not-allowed; box-shadow: none; }",
