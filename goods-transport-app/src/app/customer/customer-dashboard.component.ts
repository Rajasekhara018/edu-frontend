...
@Component({
  ...
  styles: [
    ...
    ".market-hero, .fleet-section, .booking-preview { background: var(--panel-bg); border: 1px solid var(--border); border-radius: 1.25rem; box-shadow: var(--shadow); }",
    ".market-hero { display: grid; grid-template-columns: 1.35fr 0.95fr; gap: 1rem; padding: 1.25rem; background: radial-gradient(circle at top right, color-mix(in srgb, var(--brand) 16%, transparent), transparent 26%), linear-gradient(135deg, color-mix(in srgb, var(--brand-contrast) 30%, white), color-mix(in srgb, var(--panel-bg) 86%, #eff6f1)); }",
    ".hero-copy h2 { margin: 0.35rem 0 0.6rem; font-size: clamp(1.7rem, 2.5vw, 2.8rem); color: var(--text); }",
    ".eyebrow { text-transform: uppercase; letter-spacing: 0.14em; font-size: 0.72rem; color: var(--brand-strong); font-weight: 700; }",
    ".lead, .muted, .booking-meta { color: var(--muted); }",
    ...
    ".primary { background: linear-gradient(135deg, var(--brand), var(--brand-strong)); color: var(--inverse-text); }",
    ".ghost { background: white; color: var(--text); border: 1px solid var(--border); }",
    ".hero-summary { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 0.85rem; }",
    ".summary-card, .fleet-card, .booking-card, .empty-card { background: var(--panel-muted-bg); border: 1px solid var(--border); border-radius: 1rem; padding: 1rem; }",
    ".summary-card span { color: var(--muted); display: block; }",
    ".summary-card strong { font-size: 1.5rem; color: var(--text); }",
    ...
    ".section-head h3 { margin: 0; color: var(--text); }",
    ".section-head a { color: var(--brand-strong); font-weight: 600; }",
    ".fleet-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(210px, 1fr)); gap: 1rem; }",
    ".fleet-name { margin: 0 0 0.3rem; font-weight: 700; font-size: 1.05rem; color: var(--text); }",
    ".fleet-copy { margin: 0 0 0.8rem; color: var(--muted); }",
    ".fleet-metrics { margin-bottom: 1rem; color: var(--brand-strong); font-weight: 600; }",
    ...
    ".booking-content h4 { margin: 0.2rem 0 0.4rem; color: var(--text); }",
    ".booking-side { display: flex; flex-direction: column; align-items: end; gap: 0.65rem; }",
    ".booking-side strong { font-size: 1.1rem; color: var(--text); }",
    ".booking-side a { color: var(--brand-strong); font-weight: 600; }",
